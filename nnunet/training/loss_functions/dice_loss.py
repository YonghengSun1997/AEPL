...
class DC_and_CE_loss(nn.Module):
    ...
    def forward(self, net_output, target):
        ...
        :return:
        """
        # count_zeros_ones(target)
        # count_zeros_twos(target)
...
def count_zeros_ones(tensor):
    """
    计算tensor中0和1的个数，并比较哪个更多。

    参数:
    tensor (torch.Tensor): 包含0和1的PyTorch Tensor。

    返回:
    tuple: 包含0的个数, 1的个数, 以及比较结果的字符串。
    """
    ...
    print(num_zeros, num_ones, comparison)

def count_zeros_twos(tensor):
    """
    计算tensor中0和1的个数，并比较哪个更多。
    ...
    """
    num_zeros = torch.sum(tensor == 0).item()
    num_ones = torch.sum(tensor == 2).item()
